...
# Server-side secrets and the public Supabase anon values are runtime env
# (`fly secrets set`). Docker build-args are optional; do not fail the image
# build when they are absent (Fly dashboard deploys do not pass them).
